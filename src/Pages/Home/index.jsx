...
function Home() {
  ...
  const renderView =()=>{
    if(context.search?.length >0){
      if(context.filteredItems?.length >0){
        return(
          context.filteredItems?.map(item=>( // enviar informacion de API al componente card
      ...
      return(
        context.items?.map(item=>( // enviar informacion de API al componente card
        <Card key={item.id} data={item}/>
      ))
      )
      }
  ...
  return (
      ...
      <div className='flex flex-col items-center mt-10'>
        ...
        <input type="text" placeholder="Search" className="text-center border border-black/35 rounded-lg mb-6"
