...
function CheckoutMenu() {
  ...
  const handleDelete = (id) => {
    const filterProducts = context.addProducts.filter(
  ...
  return (
    <aside
      className={`${
        context.isCheckoutMenu ? "flex" : "hidden"
      } product-checkout flex-col fixed right-0 border border-black rounded-lg bg-white`}
    >
      ...
      <div className="px-6 flex justify-between flex-1 mb-4">
        ...
        <p className="font-medium text-xl">
          $ {totalPrice(context.addProducts)}
        </p>
      </div>
